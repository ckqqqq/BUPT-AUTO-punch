...
# 服务端的记录
oldKeys = [
    "ismoved", "jhfjrq", "jhfjjtgj", "jhfjhbcc", "sfxk", "xkqq", "szgj", "szcs", "zgfxdq", "mjry", "csmjry", "uid",
    "tw", "sfcxtz", "sfyyjc", "jcjgqr", "jcjg", "sfjcbh", "sfcxzysx", "qksm", "remark", "address", "area", "province",
    "city", "geo_api_info", "sfzx", "sfjcwhry", "sfcyglq", "gllx",
    "glksrq", "jcbhlx", "jcbhrq", "sftjwh", "sftjhb", "fxyy", "bztcyy", "fjsj", "sfjchbry", "sfjcqz", "jcqzrq",
    "jcwhryfs", "jchbryfs", "xjzd", "sfsfbh", "jhfjsftjwh", "jhfjsftjhb", "szsqsfybl", "sfygtjzzfj", "gtjzzfjsj",
    "sfsqhzjkk", "sqhzjkkys", "created_uid", "gwszdd", "sfyqjzgc", "jrsfqzys", "jrsfqzfy"
]
defaultKeys = [
    "date", "created", "id",
]
# 北邮
loginAddr = "打卡登陆地址"  # "https://app.bupt.edu.cn/uc/wap/login/check
chartAddr = "打卡表格地址"  # https://app.bupt.edu.cn/ncov/wap/default/index
saveAddr = "打卡地址存储"  # https://app.bupt.edu.cn/ncov/wap/default/save
# 服务器发送邮件信息内容
...
def login(username: str, password: str):
    ...
    response = session.post(loginAddr, data={
        "username": username,
        "password": password,
    })
...
def check(username: str, password: str, mailReceiver: str):
    if login(username, password):
        ...
        return
    # 获取默认信息
    response = session.get(chartAddr)
    # print("response.text\n", response.text,"\n")
    # !!!之前忘记加原生 r "\n" gg
    ...
    # 构造返回包
    response = session.post(saveAddr, data=data)
    j = response.json()
